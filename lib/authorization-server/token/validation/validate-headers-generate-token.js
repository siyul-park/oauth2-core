...
const validateHeaderGrantTypeRefreshToken = require('./grant-type-refresh-token/validate-headers-generate-token');
...
function validateHeadersGenerateToken(params, request) {
  ...
  // eslint-disable-next-line max-len
  if (request.body.grant_type === grantType.REFRESH_TOKEN) return validateHeaderGrantTypeRefreshToken(params);
