...
async function validateAndGetCode(request) {
  ...
  // eslint-disable-next-line max-len
  if (!client || ((!!request.clientSecret || !!client.secret) && request.clientSecret !== client.secret)) {
    throw errorPool.get(UnauthorizedClientError);
  }
  ...
  }, ActiveTokenType.AUTHORIZATION_CODE);
}
