...
class Authorization {
  ...
  async allow(userId) {
    ...
    const code = new AuthorizationCode(
      { clientId: this.request.clientId, userId },
      expirationPeriod, [Scope.ACCESS_TOKEN.CREATE], this.request.scope,
    );
  ...
  async generateToken(expirationPeriod) {
    ...
    const activeTokenManager = activeTokenPool.get(ActiveTokenType.AUTHORIZATION_REQUEST_TOKEN);
