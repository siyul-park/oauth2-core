...
const responseType = require('../../response/authorization-type');

const InvalidRequestError = require('../../error/invalid-request');
const UnsupportedResponseType = require('../../error/unsupported-response-type');
...
async function validate(request) {
  ...
  validateResponseType(request.responseType, responseType.CODE,
    errorPool.get(UnsupportedResponseType));
  ...
  validateScope(ScopeType.AUTHORIZATION_CODE_GRAND_RESPONSE, request.scope, errorPool.get(InvalidScope));
